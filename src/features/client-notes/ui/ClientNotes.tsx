...
const ClientNotes: React.FC<ClientNotesProps> = ({ clientId }) => {
  ...
  const fetchNotes = async () => {
    try {
      setLoading(true)
      const response = await api.get<Note[]>(`/client-notes/client/${clientId}`)
      setNotes(response)
    } catch (err: any) {
      console.error('[ClientNotes] Error:', err)
      notifyError('Ошибка', err.message || 'Не удалось загрузить заметки')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleAddNote = async () => {
    ...
    try {
      await api.post('/client-notes', { clientId, content: newNote })
  ...
  const handleSaveEdit = async (id: number) => {
    ...
    try {
      await api.put(`/client-notes/${id}`, { content: editingContent })
            ...
            notes.map((note) => (
              ...
              >
                <Avatar className="h-10 w-10">
                  {note.user.avatar && (
                    <AvatarImage src={note.user.avatar} alt={note.user.last_name} />
                  )}
                  <AvatarFallback>
